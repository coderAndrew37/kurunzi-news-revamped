...
export const QUERIES = {
  ...
  getRelatedArticles: `
    *[_type == "post" && category->slug.current == $categorySlug && _id != $currentId] | order(publishedAt desc) [0...4] {
      title, "slug": slug.current, mainImage, publishedAt, "categorySlug": category->slug.current
    }
  `,

  getLatestArticles: `
    *[_type == "post"] | order(publishedAt desc) [0...$limit] {
      title, "slug": slug.current, "categorySlug": category->slug.current, publishedAt
    }
  `,
...
export async function fetchArticleBySlug(slug: string) {
  return await client.fetch(QUERIES.getArticleBySlug, { slug }, { next: { revalidate: 60 } });
}

export async function fetchRelatedArticles(categorySlug: string, currentId: string) {
  return await client.fetch(QUERIES.getRelatedArticles, { categorySlug, currentId }, { next: { revalidate: 3600 } });
}

export async function fetchLatestArticles(limit: number = 5) {
  return await client.fetch(QUERIES.getLatestArticles, { limit }, { next: { revalidate: 60 } });
}

export async function fetchPostsByCategory(category: string, limit: number = 8): Promise<Post[]> {
  return await client.fetch<Post[]>(QUERIES.getPostsByCategory, { category, limit }, { next: { revalidate: 60 } });
}

export async function fetchNavCategories() {
  return await client.fetch<{title: string, slug: string}[]>(QUERIES.getNavCategories);
}

export async function fetchHomepageSections(): Promise<HomepageSection[]> {
  return await client.fetch<HomepageSection[]>(QUERIES.getHomepageSections, {}, { next: { revalidate: 60 } });
}

export async function fetchCategoryLanding(slug: string) {
  // FIXED: Explicitly passing { slug } as params
  return await client.fetch(QUERIES.getCategoryLanding, { slug }, { next: { revalidate: 60 } });
}
...
export async function fetchCategoryArchive(slug: string, page: number = 1) {
  ...
  return await client.fetch(QUERIES.getCategoryArchive, { slug, start, end }, { next: { revalidate: 60 } });
}
